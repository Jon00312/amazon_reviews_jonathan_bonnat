...
logger.info("=== [ÉTAPE : TRANSFORMATION] ===")
...
def transform_all(data=None):
    """
    Étape de transformation légère pour créer la couche silver du pipeline :
    - Nettoyage et normalisation des avis
    - Enrichissements simples : image, abonnement, achat, métriques basiques
    - Export CSV propre
    ...
    logger.info("🚀 Démarrage de l'étape de transformation.")

    if data is None:
        # Utilisation en standalone, devra être séparé pour l'orchestration
        data = extract_all()

    reviews = data.get("review")
    if reviews is None:
        raise SystemExit("❌ Table 'review' absente des données extraites.")
    ...
    reviews["review_text"] = (
        ...
        .str.replace(r"[^a-zA-Z0-9À-ÿ\s.,!?']", " ", regex=True)
        .str.replace(r"\s+", " ", regex=True)
        .str.strip()
        .str.lower()
    )

    # --- Rejet des avis devenus vides après nettoyage ---
    ...
    # --- On retire les avis nettoyés mais vides ---
    ...
    if "product_reviews" in data and {"review_id", "p_id"}.issubset(data["product_reviews"].columns):
        ...
        logger.info("🔗 Colonne 'product_id' ajoutée à partir de la table 'product_reviews' (renommée depuis p_id).")
    else:
        logger.warning("⚠️ Table 'product_reviews' manquante ou colonnes absentes — 'product_id' non ajouté.")
    ...
    # --- Sélection explicite des colonnes utiles ---
    expected_cols = [
        "review_id",
        "buyer_id",
        "product_id",
    ...
    if missing_cols:
        logger.warning(f"⚠️ Colonnes absentes du DataFrame final : {missing_cols}")
    else:
        logger.info(f"✅ Colonnes finales conformes : {existing_cols}")


    # --- Export ---
    run_ts = datetime.now().strftime("%Y%m%d_%H%M%S")
    output_file = PROCESSED_DIR / f"reviews_cleaned_{run_ts}.csv"
    reviews.to_csv(output_file, index=False)
    logger.info(f"✅ Données transformées sauvegardées : {output_file}")
    logger.info(f"📊 Dataset Silver final : {len(reviews)} lignes")
    print(f"✅ {len(reviews)} avis nettoyés et enrichis. Voir logs/etl_pipeline.log pour le détail.")

    if not rejects.empty:
        logger.warning(f"⚠️ {len(rejects)} lignes rejetées détectées.")
        logger.info(f"Rejets par type : {rejects['reject_reason'].value_counts().to_dict()}")
        reject_file = REJECTS_DIR / f"reviews_rejects_{run_ts}.csv"
        rejects.to_csv(reject_file, index=False)
        logger.warning(f"✅ Rejets sauvegardés localement : {reject_file}")
    else:
        logger.info("✅ Aucune ligne rejetée.")
        
    return reviews, rejects
